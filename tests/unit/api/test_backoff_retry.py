...
from neptune_fetcher.api.client import NeptuneApiProtoClient
from neptune_fetcher.errors import NeptuneException
...
def test_retry_on_exception(response_200, sleep):
    ...
    assert NeptuneApiProtoClient._backoff_retry(func, 1, kw=2, max_tries=3) == response_200
...
def test_retry_limit_hit_on_exception(sleep):
    ...
    with pytest.raises(NeptuneException) as exc:
        NeptuneApiProtoClient._backoff_retry(func, max_tries=7)
...
def test_retry_limit_hit_on_response_error(response_500, sleep):
    ...
    with pytest.raises(NeptuneException) as exc:
        NeptuneApiProtoClient._backoff_retry(func, max_tries=5)
...
def test_retry_limit_hit_on_exception_and_response_error(response_500, sleep):
    ...
    with pytest.raises(NeptuneException) as exc:
        NeptuneApiProtoClient._backoff_retry(func, max_tries=2)
...
def test_no_error(response_200, sleep):
    ...
    assert NeptuneApiProtoClient._backoff_retry(func, 1, kw=2) == response_200
...
def test_sleep_backoff(response_500, sleep):
    ...
    with pytest.raises(NeptuneException):
        NeptuneApiProtoClient._backoff_retry(func, max_tries=10, max_backoff=10)
...
def test_unexpected_server_response():
    ...
    with pytest.raises(NeptuneException) as exc:
        NeptuneApiProtoClient._backoff_retry(func, max_tries=10)
