...
from neptune_fetcher.alpha.internal.retrieval.metrics import AttributePathInRun
...
def test_list_attributes_patched(sys_id_length, exp_count, expected_calls):
    ...
    # when
    with (
        patch("neptune_fetcher.alpha.internal.client.get_client") as get_client,
        patch("neptune_fetcher.alpha.internal.retrieval.search.fetch_sys_ids") as fetch_sys_ids,
        patch(
...
def test_fetch_experiments_table_patched(sys_id_length, exp_count, attr_name_length, attr_count, expected_calls):
    ...
    with (
        ...
        patch(
            "neptune_fetcher.alpha.internal.retrieval.attribute_values.fetch_attribute_values"
        ) as fetch_attribute_values,
    ):
        get_client.return_value = None
...
def test_fetch_metrics_patched(sys_id_length, exp_count, attr_name_length, attr_count, expected_calls):
    ...
    # when
    with (
        patch("neptune_fetcher.alpha.internal.composition.fetch_metrics.get_client") as get_client,
        patch(
            "neptune_fetcher.alpha.internal.retrieval.search.fetch_experiment_sys_attrs"
        ) as fetch_experiment_sys_attrs,
        patch(
        ...
        patch(
            "neptune_fetcher.alpha.internal.composition.fetch_metrics.fetch_multiple_series_values"
