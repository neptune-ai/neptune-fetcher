...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            r"^linear_history_root$",
            _AttributeFilter(name_matches_all=r"foo0$", aggregations=["last", "min", "max", "average", "variance"]),
            {
                "run": ["linear_history_root"],
                ("foo0:float_series", "last"): [0.1 * 9],
        ...
        (
            "^linear_history_root$",
            _AttributeFilter(name_matches_all="foo0$", aggregations=["last", "min", "max", "average", "variance"])
            | _AttributeFilter(name_matches_all=".*-value$"),
            {
                "run": ["linear_history_root"],
                ("int-value:int", ""): [1],
                ("float-value:float", ""): [1.0],
        ...
        (
            ...
            _AttributeFilter(name_matches_all=r"foo0$", aggregations=["last", "variance"]),
            {
        ...
        (
            _Filter.matches_all("sys/custom_run_id", r"forked_history_root|forked_history_fork1"),
            r".*-value$",
            {
                "run": ["forked_history_root", "forked_history_fork1"],
                ("int-value:int", ""): [1, 2],
        ...
        (
            _Filter.eq("sys/name", "exp_with_linear_history"),
        ...
        (
            _Filter.exists(_Attribute("str-value", type="string")),  # matches runs that have config 'str-value'
            r".*-value$",
            {
                "run": [
...
def test_fetch_runs_table(
    ...
    df = runs.fetch_runs_table(
        ...
        sort_by=_Attribute("sys/custom_run_id", type="string"),
        sort_direction="desc",
