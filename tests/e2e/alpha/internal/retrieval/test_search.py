...
def run_with_attributes(client, project):
    ...
    path = f"{PATH}/string-series-value"
...
@pytest.mark.parametrize(
    ...
    [
        (
            Filter.exists(
                Attribute(name=f"{PATH}/string-series-value", type="string_series"),
            ),
            True,
        ),
        (
            Filter.eq(Attribute(name=f"{PATH}/string-series-value", type="string_series"), STRING_SERIES_VALUES[-1]),
            True,
        ),
        (
            Filter.eq(
