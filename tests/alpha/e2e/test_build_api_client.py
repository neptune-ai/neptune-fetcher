...
from neptune_fetcher.alpha.api_client import BuildAuthenticatedClient


@pytest.fixture
def clear_cache_after_finishing_test():
    yield
    BuildAuthenticatedClient.clear_cache()


def test_api_client_cache_works(clear_cache_after_finishing_test):
    client = BuildAuthenticatedClient.build()
    client2 = BuildAuthenticatedClient.build()
...
def test_api_client_cache_works_for_separate_context(clear_cache_after_finishing_test):
    ...
    client = BuildAuthenticatedClient.build(ctx)
    client2 = BuildAuthenticatedClient.build(ctx)
    assert client2 == client

    ctx2 = npt.Context(project="random_non_existing_test_project")
    client3 = BuildAuthenticatedClient.build(ctx2)
    assert client3 == client

    # only api token matters, so different project are irrelevant with regard to ApiClient itself
    client_default = BuildAuthenticatedClient.build()
