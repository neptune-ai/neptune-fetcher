...
| `match_columns_to_filters` | `bool`, optional                              | `False`             | DEPRECATED: The argument is now assumed to be always `True` and will be removed in a future release.<br/><br/>If `True`, the columns regex will only match columns that are present in the runs that pass the run filters. The run filters must match up to 5000 entries. If `False`, the columns regex will match all columns in the project.                                                                                               |
