...
### Changes
- Add support for point previews when fetching metric values ([#226](https://github.com/neptune-ai/neptune-fetcher/pull/226)). **Note:** This change introduces the `include_point_previews` parameter to the `fetch_metrics()` function. The new parameter is in the middle of the list. If you've used the arguments of this function positionally, ensure that the order is valid or use keywords.
